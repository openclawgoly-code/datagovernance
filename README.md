...
单元测试之外,有九份打**真实运行中的应用**的端到端验收脚本,逐条核对各阶段的
完成判据。它们可反复执行,连接参数全部从环境变量读:

| 脚本 | 断言数 | 覆盖 |
|---|---|---|
| `verify-p1.py` | 47 | 数据源、连通性、目录、租户隔离、多节点 MPP、空间启停 |
| `verify-p2.py` | 103 | 编译诊断、Cron、真实搬 2500 行、整库迁移、文件/接口解析、规则 |
...
| `verify-p6-datasets.py` | 44 | 公开数据集:分区表结构探测、异构类型保真、跨方言整库迁移对照、GBK 中文文件解析 |
| `verify-p7-concurrency.py` | 21 | 并发下执行事实不串、真实数据量取消、线程池过载归因 |
| `verify-p8-retry.py` | 13 | 写到一半失败后重投的幂等性、失败尝试的写入行数 |
| `verify-p9-upsert.py` | 25 | 按主键写入:更新与插入同时发生、复合主键、无约束时报错、跨方言一致 |
...
*修复分两步*。第一步先让失败发生在配置时而不是数据里:编译期把 `UPSERT` 单独
拦下,报「本版本尚未实现」而不是混进「写入模式无效」——那句话会让用户以为自己
拼错了。第二步把它真正实现了,见下一节。

### 按主键写入验收(P9)

`UPSERT` 是本仓库里**唯一一种六个方言写法各不相同**的写入模式:

| 方言 | 写法 |
|---|---|
| PostgreSQL | `INSERT ... ON CONFLICT (pk) DO UPDATE SET` |
| MySQL | `INSERT ... ON DUPLICATE KEY UPDATE`(用 `VALUES(col)` 取待插入的值) |
| Oracle / 达梦 | `MERGE INTO ... USING (SELECT ? FROM dual) ... WHEN MATCHED` |
| SQL Server | `MERGE INTO ... USING (VALUES (?)) ...`,**必须以分号结尾** |
| Doris / StarRocks | **普通 INSERT** —— 它们没有 upsert 语法 |

最后一行是这套实现里唯一「语句正确但结果仍可能不对」的情形:Doris 与 StarRocks
按主键覆盖靠的是**表模型**(UNIQUE KEY / 主键模型),目标表若建成了明细模型,
任务会成功、数据会重复、没有任何报错。所以编译期对这两种目标端专门发一条提醒。

生成的语句集中在 `DialectUpsert`,与 `DialectDdl` 并排——两者是同一件事的两半。
三条跨方言的共同约定由参数化测试钉住:

- **参数顺序恒等于 columns**,每个值只绑一次。MERGE 之所以也能做到,是因为值先进
  `USING` 子句成为一行「源」,后面的 UPDATE 与 INSERT 都引用它而不再重复绑定。
  否则调用方要为每种方言记一套不同的绑定顺序,那是必然出错的地方。
- **主键不出现在 UPDATE 的赋值里**。Oracle 明令禁止(ORA-38104:ON 子句里的列
  不能被更新),其余方言允许但没有意义——更新主键等于把这一行变成另一行。
- **所有列都是主键时各有各的退路**:PostgreSQL `DO NOTHING`、MySQL 拿主键给自己
  赋值、三种 MERGE 只留 `WHEN NOT MATCHED` 分支。空的 `UPDATE SET` 是语法错误,
  而「全是主键」的维度表很常见。

```bash
DG_ADMIN_PASSWORD='换成你的口令' python3 scripts/verify-p9-upsert.py
# 跨方言那一节要 MySQL;不起则整段跳过
./scripts/start-test-mysql.sh
```

语句生成那一半是纯字符串操作,由 `DialectUpsertTest` 逐方言钉死。P9 补的是另一半:
**生成的语句拿到真库上跑,结果对不对**。判据不是「任务成功」,而是三件必须同时
成立的事——总行数没有因为重复写而变多、本来就存在的行**被更新了**、原本没有的行
**被插入了**。只验第一条会漏掉「退化成 `DO NOTHING`」,只验第三条会漏掉
「退化成 APPEND」。

两节是这份脚本的重点:

- **跨方言**:同一份任务配置分别打到 PostgreSQL 与 MySQL,生成的语句完全不同,
  结果必须一样。单方言跑通证明不了方言分支写对了。
- **目标表没有唯一约束时失败要响**:PostgreSQL 报 42P10 而不是静默退化成追加,
  且一行都没写进去。这一条不成立的话,编译期那条提醒就只是句空话。

**说清楚验到哪儿为止**:Oracle、SQL Server、达梦在本仓库的验证环境里没有实例,
它们**只有单元测试看着**——语句形状是对的,但没有在真实引擎上跑过。这三种方言
的断言因此写得比别处细(连 SQL Server 的结尾分号都单独一条),但它们证明的是
「生成的字符串符合预期」,不是「目标端接受它」。

顺带,UPSERT 实现之后成了**第二种幂等的写入模式**:同一主键写第二遍是覆盖而不是
新增,所以整批重放一遍结果不变。P8 因此多了一节——UPSERT 写到一半失败时,重试
照旧发生三次,而目标表里依然没有重复行。它曾经生成普通 INSERT 的那段历史,正是
这一节存在的理由:若哪天又退回那样,这一节会红。
