...
> **一个面向政务医疗场景、以 Workspace 为租户隔离边界、以 Metadata Kernel 为唯一真相来源、以 Workflow Compiler 为控制中枢、以 Flink + Kubernetes 为统一执行底座的数据集成与数据开发平台;向上提供多源异构数据接入与同步、Flink SQL 实时/离线开发与可视化工作流编排、跨模块统一的任务监控告警与操作审计,并为后续的医学高质量数据集制备预留语义与数据集契约。**
...
| `ExternalPlatformAdapter` | 全民健康信息平台对接适配器 | 35 |
...
政务项目常见的交付硬件是 4C8G × 3 台。7 个 Spring Boot 进程各自占用 JVM 堆、连接池与线程池,在这个规格上会明显吃紧,而 Phase 1 的业务量根本用不上这种隔离。
...
**约束三:政务医疗交付场景。**
等保测评、第三方安全扫描、甲方运维接手、项目期内人员替换 —— Java + Spring Boot 是这个场景的默认答案。信创环境下毕昇 JDK、龙芯 JDK 均成熟;Rust 在龙芯 LoongArch 上的工具链支持要弱一截。这是交付风险,不是技术优劣。
...
| 身份认证 | Keycloak | **对接甲方已有统一身份认证**(OIDC / CAS) | 政务场景通常不允许自建账号体系,序号 30 需澄清 |
...
**(2) 用户体系是自建还是对接?**
序号 30 描述了完整的用户增删改查与重置密码,读起来像自建。但政务医疗场景通常要求对接已有统一身份认证。若是对接,序号 30 的多数功能应改为「用户同步 + 本地授权」。
...
**范围**:`intelligence-platform`(独立立项) + 全民健康信息平台对接
**功能项**:34、35
...
| **R8** | 序号 35 是外部依赖,接口规范未知;健康医疗数据属敏感个人信息 | 排期把它当作普通功能项 | 适配器隔离在 Data Space 边缘;脱敏、授权、留痕由 Governance 强制;**按外部依赖单列里程碑,不与内部功能混排** |
...
4. **用户体系是自建还是对接甲方统一认证?**(L.2)—— 序号 30 读起来像自建,但政务医疗场景通常要求对接
